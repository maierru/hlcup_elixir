# https://github.com/kennyballou/docker-elixir-centos
FROM centos:7
...
RUN cd /usr/src \
    ...
    && cd otp \
    && git fetch origin pull/1552/head:highload \
    && git checkout highload \
    && ./otp_build autoconf && ./configure && make && make install
...
# RUN REBAR_SRC_URL="https://github.com/rebar/rebar/archive/${REBAR_VERSION}.tar.gz" \
...
COPY . .
EXPOSE 80
ENV PORT=80 MIX_ENV=prod TERM=xterm
